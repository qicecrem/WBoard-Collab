...
WBMagnifier::WBMagnifier(QWidget *parent, bool isInteractive)
...
void WBMagnifier::setSize(qreal percentFromScene)
{
...
void WBMagnifier::setZoom(qreal zoom)
{
    params.zoom = zoom;
}


void WBMagnifier::calculateButtonsPositions()
{
...
void WBMagnifier::mousePressEvent ( QMouseEvent * event )
{
    if(m_isInteractive)
    {
...
void WBMagnifier::mouseMoveEvent ( QMouseEvent * event )
{
    if(m_isInteractive)
    {
...
void WBMagnifier::mouseReleaseEvent(QMouseEvent * event)
{
    if(m_isInteractive)
    {
        mShouldMoveWidget = false;
        ...
        if (sChangeModePixmapButtonRect.contains(event->position().toPoint()))
        {
            ...
            QWidget::mouseReleaseEvent(event); // don't propgate to parent, the widget is deleted in WBApplication::boardController->removeTool
    }
    else
        event->ignore();
...
{
    QMatrix transM = WBApplication::boardController->controlView()->matrix();
    ...
    QPointF pfScLtF(WBApplication::boardController->controlView()->mapToScene(QPoint(itemPos.x(), itemPos.y())));
    ...
    WBApplication::boardController->activeScene()->render(&painter, QRectF(0,0,width(),height()), srcRect);
    painter.end();
...
// from global
void WBMagnifier::grabNMove(const QPoint &pGrab, const QPoint &pMove, bool needGrab, bool needMove)
{
...
void WBMagnifier::setGrabView(QWidget *view)
{
    gView = view;
...
void WBMagnifier::setDrawingMode(int mode)
{
    ...
    WBSettings::settings()->magnifierDrawingMode->set(mode);
}
