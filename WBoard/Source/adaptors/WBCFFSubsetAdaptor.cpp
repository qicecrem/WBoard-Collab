...
WBCFFSubsetAdaptor::WBCFFSubsetAdaptor()
{}

bool WBCFFSubsetAdaptor::ConvertCFFFileToWbz(QString &cffSourceFile, WBDocumentProxy* pDocument)
{
    //TODO
    // fill document proxy metadata
    // create persistance manager to save data using proxy
    // create WBCFFSubsetReader and make it parse cffSourceFolder
    ...
    WBCFFSubsetReader cffReader(pDocument, &file);
    bool result =  cffReader.parse();
    file.close();

    return result;
}
WBCFFSubsetAdaptor::WBCFFSubsetReader::WBCFFSubsetReader(WBDocumentProxy *proxy, QFile *content)
    : mProxy(proxy)
...
{
    WBMetadataDcSubsetAdaptor::persist(mProxy);
...
//        WBFileSystemUtils::deleteDir(mTmpFlashDir.path());

    return result;
}

bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseGSection(const QDomElement &element)
{
    mGSectionContainer = new WBGraphicsGroupContainerItem();
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgSwitchSection(const QDomElement &element)
{
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgRect(const QDomElement &element)
{
    ...
    WBGraphicsSvgItem *svgItem = mCurrentScene->addSvg(QUrl::fromLocalFile(generator->fileName()));
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgEllipse(const QDomElement &element)
{
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgPolygon(const QDomElement &element)
{
    ...
    QUuid itemUuid = QUuid::fromString(element.attribute(aId).right(QUuid().toString().length()));
    QUuid itemGroupUuid = QUuid::fromString(element.attribute(aId).left(QUuid().toString().length()-1));
    if (!itemUuid.isNull() && (itemGroupUuid!=itemUuid)) // reimported from UBZ
    {
        WBGraphicsPolygonItem *graphicsPolygon = mCurrentScene->polygonToPolygonItem(polygon);
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgPolyline(const QDomElement &element)
{
    QString svgPoints = element.attribute(aPoints);
    QPolygonF polygon;
    ...
    if (!itemUuid.isNull() && (itemGroupUuid!=itemUuid)) // reimported from UBZ
    {
        WBGraphicsPolygonItem *graphicsPolygon = new WBGraphicsPolygonItem(polygon);

        WBGraphicsStroke *stroke = new WBGraphicsStroke();
...
void WBCFFSubsetAdaptor::WBCFFSubsetReader::parseTextAttributes(const QDomElement &element,
...
void WBCFFSubsetAdaptor::WBCFFSubsetReader::readTextBlockAttr(const QDomElement &element, QTextBlockFormat &format)
{
    QString fontStretchText = element.attribute(aFontstretch);
...
void WBCFFSubsetAdaptor::WBCFFSubsetReader::readTextCharAttr(const QDomElement &element, QTextCharFormat &format)
{
    QString fontSz = element.attribute(aFontSize);
    if (!fontSz.isNull()) {
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgText(const QDomElement &element)
{
...
void WBCFFSubsetAdaptor::WBCFFSubsetReader::parseTSpan(const QDomElement &parent, QPainter &painter
...
void WBCFFSubsetAdaptor::WBCFFSubsetReader::parseTSpan(const QDomElement &element, QTextCursor &cursor
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgTextarea(const QDomElement &element)
{
    qreal x = element.attribute(aX).toDouble();
    qreal y = element.attribute(aY).toDouble();
    qreal width = element.attribute(aWidth).toDouble();
    ...
    WBGraphicsTextItem *svgItem = mCurrentScene->addTextHtml(doc.toHtml());
    ...
    WBGraphicsTextItemDelegate *curDelegate = dynamic_cast<WBGraphicsTextItemDelegate*>(svgItem->Delegate());
    if (curDelegate) {
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgImage(const QDomElement &element)
{
    qreal x = element.attribute(aX).toDouble();
    qreal y = element.attribute(aY).toDouble();
    qreal width = element.attribute(aWidth).toDouble();
...
//            qDebug() << "size of file" << itemRefPath << QFileInfo(itemRefPath).size();
   ...
   WBGraphicsPixmapItem *pixItem = mCurrentScene->addPixmap(pix, NULL);
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgFlash(const QDomElement &element)
{
    QString itemRefPath = element.attribute(aHref);
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgAudio(const QDomElement &element)
{
    ...
    QString destFile;
    bool b = WBPersistenceManager::persistenceManager()->addFileToDocument(
    ...
    WBGraphicsMediaItem *audioItem = mCurrentScene->addAudio(concreteUrl, false);
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgVideo(const QDomElement &element)
{
    QString itemRefPath = element.attribute(aHref);
    ...
    WBGraphicsMediaItem *videoItem = mCurrentScene->addVideo(concreteUrl, false);
...
void WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgSectionAttr(const QDomElement &svgSection)
{
...
void WBCFFSubsetAdaptor::WBCFFSubsetReader::addItemToGSection(QGraphicsItem *item)
{
    mGSectionContainer->addToGroup(item);
}

void WBCFFSubsetAdaptor::WBCFFSubsetReader::hashSceneItem(const QDomElement &element, WBGraphicsItem *item)
{
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgElement(const QDomElement &parent)
{
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgPage(const QDomElement &parent)
{
    createNewScene();
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseSvgPageset(const QDomElement &parent)
{
...
WBGraphicsGroupContainerItem *WBCFFSubsetAdaptor::WBCFFSubsetReader::parseIwbGroup(QDomElement &parent)
{
    ...
    QMultiMap<QString, WBGraphicsPolygonItem *> strokesGroupsContainer;    
        ...
        else
        {
            ...
            if (!uuid.isEmpty())
            {
                ...
                {              
                    ...
                    WBGraphicsPolygonItem *strokeByUuid = qgraphicsitem_cast<WBGraphicsPolygonItem *>(mCurrentScene->itemForUuid(QUuid(ref.right(QUuid().toString().length()))));
    ...
    foreach (QString key, QSet<QString>(strokesGroupsContainer.keys().begin(), strokesGroupsContainer.keys().end()))
    {
        ...
        foreach(WBGraphicsPolygonItem* poly, strokesGroupsContainer.values(key))
        {
            if (poly)
            {
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::strToBool(QString str)
{
    return str == "true";
}
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseIwbElement(QDomElement &element)
{
    ...
    bool isEditable = false; //Text items to convert to WBGraphicsTextItem only
    ...
    if (!IDRef.isNull()) {
        ...
        WBGraphicsItem *referedItem(0);
        QHash<QString, WBGraphicsItem*>::iterator iReferedItem;
        ...
        if (isEditableItem) {
            WBGraphicsTextItemDelegate *textDelegate = dynamic_cast<WBGraphicsTextItemDelegate*>(referedItem->Delegate());
            if (textDelegate) {
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::parseDoc()
{
...
void WBCFFSubsetAdaptor::WBCFFSubsetReader::repositionSvgItem(QGraphicsItem *item, qreal width, qreal height,
                                                              qreal x, qreal y,
                                                              QTransform &transform)
{
    ...
    QPoint pos;
    if (WBGraphicsTextItem::Type == item->type())
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::createNewScene()
{
    mCurrentScene = WBPersistenceManager::persistenceManager()->createDocumentSceneAt(mProxy, mProxy->pageCount(), false);
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::persistCurrentScene()
{
    if (mCurrentScene != 0 && mCurrentScene->isModified())
    {
        WBThumbnailAdaptor::persistScene(mProxy, mCurrentScene, mProxy->pageCount() - 1);
        WBSvgSubsetAdaptor::persistScene(mProxy, mCurrentScene, mProxy->pageCount() - 1);
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::persistScenes()
{
    ...
    for (int i = 0; i < mProxy->pageCount(); i++) {
        ...
        WBSvgSubsetAdaptor::persistScene(mProxy, mCurrentScene, i);
        WBGraphicsScene *tmpScene = WBSvgSubsetAdaptor::loadScene(mProxy, i);
        tmpScene->setModified(true);
        WBThumbnailAdaptor::persistScene(mProxy, tmpScene, i);
        mCurrentScene->setModified(false);
    }
...
QColor WBCFFSubsetAdaptor::WBCFFSubsetReader::colorFromString(const QString& clrString)
{
...
QTransform WBCFFSubsetAdaptor::WBCFFSubsetReader::transformFromString(const QString trString, QGraphicsItem *item)
{
    qreal dxr = 0.0;
    qreal dyr = 0.0;
    qreal dx = 0.0;
...
QSvgGenerator* WBCFFSubsetAdaptor::WBCFFSubsetReader::createSvgGenerator(qreal width, qreal height)
{
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::getTempFileName()
{
    int tmpNumber = 0;
    QDir rootDir;
    while (true)
...
bool WBCFFSubsetAdaptor::WBCFFSubsetReader::createTempFlashPath()
{
    int tmpNumber = 0;
...
WBCFFSubsetAdaptor::WBCFFSubsetReader::~WBCFFSubsetReader()
{
//    QList<int> pages;
//    for (int i = 0; i < mProxy->pageCount(); i++) {
//        pages << i;
//    }
//    WBPersistenceManager::persistenceManager()->deleteDocumentScenes(mProxy, pages);
}
