...
void DelegateButton::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (mIsPressed && mShowProgressIndicator) {
        ...
        int spanAngle = qMin(mPressProgres, WBSettings::longClickInterval) * 360 / WBSettings::longClickInterval;
...
WBGraphicsItemDelegate::WBGraphicsItemDelegate(QGraphicsItem* pDelegated, QObject * parent, WBGraphicsFlags fls)
    : QObject(parent)
    , mDelegated(pDelegated)
    , mDeleteButton(NULL)
...
{
    setUBFlags(fls);
    connect(WBApplication::boardController, SIGNAL(zoomChanged(qreal)), this, SLOT(onZoomChanged()));
}

void WBGraphicsItemDelegate::createControls()
{
    if (testUBFlags(GF_TOOLBAR_USED) && !mToolBarItem)
        mToolBarItem = new WBGraphicsToolBarItem(mDelegated);

    if (!mFrame) {
        mFrame = new WBGraphicsDelegateFrame(this, QRectF(0, 0, 0, 0), mFrameWidth, testUBFlags(GF_RESPECT_RATIO), testUBFlags(GF_TITLE_BAR_USED));
        mFrame->hide();
...
WBGraphicsItemDelegate::~WBGraphicsItemDelegate()
{
    if (WBApplication::boardController)
        disconnect(WBApplication::boardController, SIGNAL(zoomChanged(qreal)), this, SLOT(onZoomChanged()));
...
QVariant WBGraphicsItemDelegate::itemChange(QGraphicsItem::GraphicsItemChange change, const QVariant &value)
{
    WBGraphicsScene *ubScene = castUBGraphicsScene();
    ...
    {
        bool shownOnDisplay = mDelegated->data(WBGraphicsItemData::ItemLayerType).toInt() != WBItemLayerType::Control;
...
WBGraphicsScene *WBGraphicsItemDelegate::castUBGraphicsScene()
{
    WBGraphicsScene *castScene = dynamic_cast<WBGraphicsScene*>(delegated()->scene());

    return castScene;
}

/** Used to render custom data after the main "Paint" operation is finished */
void WBGraphicsItemDelegate::postpaint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget)
...
bool WBGraphicsItemDelegate::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
...
void WBGraphicsItemDelegate::setMimeData(QMimeData *mimeData)
{
    mMimeData = mimeData;
}

bool WBGraphicsItemDelegate::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    if(mMimeData) {
...
bool WBGraphicsItemDelegate::wheelEvent(QGraphicsSceneWheelEvent *event)
{
    Q_UNUSED(event);
    if( delegated()->isSelected() )
        return true;
...
bool WBGraphicsItemDelegate::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event);
...
void WBGraphicsItemDelegate::hoverEnterEvent(QGraphicsSceneHoverEvent *event)
{
    Q_UNUSED(event)

}

void WBGraphicsItemDelegate::hoverLeaveEvent(QGraphicsSceneHoverEvent *event)
{
    Q_UNUSED(event)

}

bool WBGraphicsItemDelegate::keyPressEvent(QKeyEvent *event)
{
    Q_UNUSED(event);
    return true;
}

bool WBGraphicsItemDelegate::keyReleaseEvent(QKeyEvent *event)
{
    Q_UNUSED(event);
    return true;
}

QGraphicsItem *WBGraphicsItemDelegate::delegated()
{
    QGraphicsItem *curDelegate = 0;
    if (mDelegated->parentItem() && mDelegated->parentItem()->type() == WBGraphicsGroupContainerItem::Type) {
...
void WBGraphicsItemDelegate::positionHandles()
{
    if (!controlsExist()) {
        return;
    }
    ...
    if (mDelegated->isSelected()) {
        bool shownOnDisplay = mDelegated->data(WBGraphicsItemData::ItemLayerType).toInt() != WBItemLayerType::Control;
        showHide(shownOnDisplay);
        mDelegated->setData(WBGraphicsItemData::ItemLocked, QVariant(isLocked()));
        updateFrame();
...
void WBGraphicsItemDelegate::setZOrderButtonsVisible(bool visible)
{
    if (visible) {
        updateFrame();
        updateButtons();
...
void WBGraphicsItemDelegate::remove(bool canUndo)
{
    WBGraphicsScene* scene = dynamic_cast<WBGraphicsScene*>(mDelegated->scene());
    if (scene)
    {
        ...
        WBGraphicsWidgetItem *mDelegated_casted = dynamic_cast<WBGraphicsWidgetItem*>(mDelegated);
  /*      if (mDelegated_casted)
            mDelegated_casted->setHtml(QString());*///zhusizhi

        scene->removeItem(mDelegated);

        if (canUndo)
        {
            WBGraphicsItemUndoCommand *uc = new WBGraphicsItemUndoCommand(scene, mDelegated, 0);
...
bool WBGraphicsItemDelegate::isLocked() const
{
    return mDelegated->data(WBGraphicsItemData::ItemLocked).toBool();
}


void WBGraphicsItemDelegate::duplicate()
{
    WBApplication::boardController->duplicateItem(dynamic_cast<WBItem*>(delegated()));
}

void WBGraphicsItemDelegate::increaseZLevelUp()
{
    WBGraphicsScene *curScene = castUBGraphicsScene();
    if (curScene) {
        curScene->changeZLevelTo(delegated(), WBZLayerController::up, true);
    }
}
void WBGraphicsItemDelegate::increaseZlevelTop()
{
    WBGraphicsScene *curScene = castUBGraphicsScene();
    if (curScene) {
        curScene->changeZLevelTo(delegated(), WBZLayerController::top, true);
    }
}
void WBGraphicsItemDelegate::increaseZLevelDown()
{
    WBGraphicsScene *curScene = castUBGraphicsScene();
    if (curScene) {
        curScene->changeZLevelTo(delegated(), WBZLayerController::down, true);
    }
}
void WBGraphicsItemDelegate::increaseZlevelBottom()
{
    WBGraphicsScene *curScene = castUBGraphicsScene();
    if (curScene) {
        curScene->changeZLevelTo(delegated(), WBZLayerController::bottom, true);
    }
}

void WBGraphicsItemDelegate::lock(bool locked)
{
...
void WBGraphicsItemDelegate::setLockedRecurs(const QVariant &pLock, QGraphicsItem *pItem)
{
    pItem->setData(WBGraphicsItemData::ItemLocked, pLock);
...
void WBGraphicsItemDelegate::showHide(bool show)
{
...
void WBGraphicsItemDelegate::showHideRecurs(const QVariant &pShow, QGraphicsItem *pItem)
{
    pItem->setData(WBGraphicsItemData::ItemLayerType, pShow);
    foreach (QGraphicsItem *insideItem, pItem->childItems()) {
 ...
 */
void WBGraphicsItemDelegate::setAsBackground()
{
    WBGraphicsScene* scene = castUBGraphicsScene();
    ...
    if (scene && item) {
        ...
        WBGraphicsItemTransformUndoCommand *uc =
                new WBGraphicsItemTransformUndoCommand(mDelegated,
                                                       mPreviousPosition,
                                                       mPreviousTransform,
                                                       mPreviousZValue,
                                                       mPreviousSize,
...
void WBGraphicsItemDelegate::gotoContentSource()
{
    WBItem* item = dynamic_cast<WBItem*>(mDelegated);

    if(item && !item->sourceUrl().isEmpty())
    {
        WBApplication::applicationController->showInternet();
        WBApplication::webController->loadUrl(item->sourceUrl());
    }
}

void WBGraphicsItemDelegate::startUndoStep()
{
    mPreviousPosition = mDelegated->pos();
    mPreviousTransform = mDelegated->transform();
    mPreviousZValue = mDelegated->zValue();
    WBResizableGraphicsItem* resizableItem = dynamic_cast<WBResizableGraphicsItem*>(mDelegated);
...
void WBGraphicsItemDelegate::commitUndoStep()
{
    WBResizableGraphicsItem* resizableItem = dynamic_cast<WBResizableGraphicsItem*>(mDelegated);
...
void WBGraphicsItemDelegate::onZoomChanged()
{
    mAntiScaleRatio = 1 / (WBApplication::boardController->systemScaleFactor() * WBApplication::boardController->currentZoom());
...
void WBGraphicsItemDelegate::buildButtons()
{
}

void WBGraphicsItemDelegate::freeButtons()
{
...
void WBGraphicsItemDelegate::decorateMenu(QMenu* menu)
{
    ...
    if (delegated()->data(WBGraphicsItemData::ItemCanBeSetAsBackground).toBool()) {
...
void WBGraphicsItemDelegate::updateMenuActionState()
{
    if (mLockAction)
    ...
    {
        bool isControl = mDelegated->data(WBGraphicsItemData::ItemLayerType) == WBItemLayerType::Control;
...
void WBGraphicsItemDelegate::showMenu()
{
    if (!mMenu)
    {
        mMenu = new QMenu(WBApplication::boardController->controlView());
        decorateMenu(mMenu);
    }

    updateMenuActionState();

    WBBoardView* cv = WBApplication::boardController->controlView();
    QRect pinPos = cv->mapFromScene(mMenuButton->sceneBoundingRect()).boundingRect();

    mMenu->exec(cv->mapToGlobal(pinPos.bottomRight()));
}

void WBGraphicsItemDelegate::setLocked(bool pLocked)
{
    Q_ASSERT(mDelegated);

    if (mDelegated) {
        mDelegated->setData(WBGraphicsItemData::ItemLocked, QVariant(pLocked));
    }
}

void WBGraphicsItemDelegate::updateFrame()
{
    if (mFrame && !mFrame->scene() && mDelegated->scene())
        mDelegated->scene()->addItem(mFrame);
...
void WBGraphicsItemDelegate::updateButtons(bool showUpdated)
{
    QTransform tr;
...
void WBGraphicsItemDelegate::setButtonsVisible(bool visible)
{
...
void WBGraphicsItemDelegate::setUBFlags(WBGraphicsFlags pf)
{
    mFlags = pf;
    ...
    if (testUBFlags(GF_FLIPPABLE_ALL_AXIS)) {
        mDelegated->setData(WBGraphicsItemData::ItemFlippable, QVariant(testUBFlags(GF_FLIPPABLE_ALL_AXIS)));
    }

    if (testUBFlags(GF_REVOLVABLE)) {
        mDelegated->setData(WBGraphicsItemData::ItemRotatable, QVariant(testUBFlags(GF_REVOLVABLE)));
    }
}

void WBGraphicsItemDelegate::setUBFlag(WBGraphicsFlags pf, bool set)
{
    WBGraphicsFlags fls = mFlags;
    set ? fls |= pf : fls &= ~pf;

    setUBFlags(fls);
}

WBGraphicsToolBarItem::WBGraphicsToolBarItem(QGraphicsItem * parent) :
...
void WBGraphicsToolBarItem::positionHandles()
{
    int itemXOffset = 0;
...
void WBGraphicsToolBarItem::update()
{
    QGraphicsRectItem::update();
}

void WBGraphicsToolBarItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
...
DelegateMediaControl::DelegateMediaControl(WBGraphicsMediaItem* pDelegated, QGraphicsItem * parent)
    : QGraphicsRectItem(parent)
        ...
        const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    painter->setBrush(WBSettings::documentViewLightColor);
