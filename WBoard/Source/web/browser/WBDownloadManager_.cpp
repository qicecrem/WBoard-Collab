...
void WBDownloadItem::tryAgain()
{
    ...
    QNetworkReply *r = WBNetworkAccessManager::defaultAccessManager()->get(QNetworkRequest(m_url));
    if (m_reply)
        m_reply->deleteLater();
    if (m_output.exists())
        m_output.remove();
...
void WBDownloadItem::finished()
{
    ...
    if(m_output.fileName().toLower().endsWith(".wgt") || m_output.fileName().toLower().endsWith(".wdgt")){
        ...
        if (!QFileInfo(m_output.fileName()).isDir()){
            WBFileSystemUtils::expandZipToDir(m_output.fileName(), destPath);
            m_output.remove();
        }
    }
    emit statusChanged();
...
/*!
  ...
  */
WBDownloadManager_::WBDownloadManager_(QWidget *parent)
    : QDialog(parent)
    , mAutoSaver(new WBAutoSaver(this))
    , mManager(WBNetworkAccessManager::defaultAccessManager())
    , mIconProvider(0)
...
WBDownloadManager_::~WBDownloadManager_()
{
...
int WBDownloadManager_::activeDownloads() const
{
    int count = 0;
...
void WBDownloadManager_::download(const QNetworkRequest &request, bool requestFileName)
{
...
void WBDownloadManager_::handleUnsupportedContent(QNetworkReply *reply, bool requestFileName, QString customDownloadPath)
{
...
void WBDownloadManager_::processDownloadedContent(QNetworkReply *reply, bool requestFileName, QString customDownloadPath)
{
...
void WBDownloadManager_::addItem(WBDownloadItem *item)
{
...
void WBDownloadManager_::updateRow()
{
    ...
    if (item->downloadedSuccessfully() && removePolicy() == WBDownloadManager_::SuccessFullDownload)
        remove = true;
...
WBDownloadManager_::RemovePolicy WBDownloadManager_::removePolicy() const
{
    return m_RemovePolicy;
}


void WBDownloadManager_::setRemovePolicy(RemovePolicy policy)
{
...
void WBDownloadManager_::save() const
{
    QSettings settings;
    settings.beginGroup(QLatin1String("downloadmanager"));
...
void WBDownloadManager_::cleanup()
{
...
void WBDownloadManager_::updateItemCount()
{
...
WBDownloadModel::WBDownloadModel(WBDownloadManager_ *downloadManager, QObject *parent)
