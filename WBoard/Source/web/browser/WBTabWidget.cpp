    ...
    , mTabBar(new WBTabBar(this))
{
	...
	mLineEdits->setStyleSheet("background-color:rgba(255,255,255,127)");
}
...
WBWebView *WBTabWidget::newTab(bool makeCurrent)
{
    ...
    connect(webView, SIGNAL(pixmapCaptured(const QPixmap&, bool)), WBApplication::applicationController, SLOT(addCapturedPixmap(const QPixmap &, bool)));

    connect(webView, SIGNAL(embedCodeCaptured(const QString&)), WBApplication::applicationController, SLOT(addCapturedEmbedCode(const QString&)));
